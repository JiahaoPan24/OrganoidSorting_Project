...
class Window(Program):
    ...
    def Description(self):
        return "Identifies and analyzes organoids in on-screen window."
    ...
    def SetupParser(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument("windowName", help="Name of window to run model on.",
                            type=str)
        parser.add_argument("classificationPath", help="Path to classification model.",
                            type=pathlib.Path, nargs='?', default=None)
        parser.add_argument("-T", dest="threshold", default=0.6,
        ...
        parser.add_argument("-ES", dest="edgeSigma", default=2,
                            help="Standard deviation of Gaussian kernel used for edge "
                                 "detection.",
                            type=float)
        parser.add_argument("-EL", dest="edgeMin", default=0.01,
    ...
    def RunProgram(self, parserArgs: argparse.Namespace):
        from Core.WindowPipeline import WindowPipeline

        WindowPipeline(parserArgs.modelPath, parserArgs.windowName, parserArgs.classificationPath,
                    ...
                    True, not parserArgs.border, parserArgs.belief,
