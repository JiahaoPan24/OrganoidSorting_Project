...
class Run(Program):
    ...
    def SetupParser(self, parser: argparse.ArgumentParser):
        ...
        parser.add_argument("-A", dest="minimumArea", default=100,
                            help="Ignore objects with an area smaller than this number of "
                                 "pixels.",
                            type=int)
        parser.add_argument("--classPath", default=None, help="Path to trained classification model.", type=pathlib.Path)
        parser.add_argument("--nofill", action="store_true",
                            help="If set, holes in detected objects will not be filled.")
        parser.add_argument("--run_class", action="store_true",
                            help="If set, classification will run.")
    ...
    def RunProgram(self, parserArgs: argparse.Namespace):
        ...
        RunPipeline(parserArgs.run_class, parserArgs.modelPath, parserArgs.imagesPath, parserArgs.outputPath, parserArgs.classPath,
