...
def LabelAndBoundingBox(images: np.ndarray, foregroundThreshold, padding: int = 10):
    labeled = np.zeros(images.shape, np.uint16)
    bounding_boxes = []
    ...
    for i in range(images.shape[0]):
        ...
        # Calculating bounding boxes for each labeled region (organoid)
        regions = skimage.measure.regionprops(labeled[i])
        for region in regions:
            # Add padding to the bounding box
            minr, minc, maxr, maxc = region.bbox
            minr = max(minr - padding, 0)
            minc = max(minc - padding, 0)
            maxr = min(maxr + padding, images.shape[1])
            maxc = min(maxc + padding, images.shape[2])
            bounding_boxes.append((minr, minc, maxr, maxc))
    
    return labeled, bounding_boxes
