...
def draw_ids_on_image(image, mapping):
    pil_image = Image.fromarray(image)
    draw = ImageDraw.Draw(pil_image)
    font = ImageFont.load_default()

    for label, track_id in mapping:
        region = skimage.measure.regionprops(image == label)[0]
        centroid = region.centroid
        draw.text(centroid[::-1], str(track_id), fill=(255, 0, 0), font=font)
